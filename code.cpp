...
private:
    ...
    bool isValidEmail(const string &email) {
        const string domain = "@mail.jiit.ac.in";
...
public:
    ...
    bool registerUser() {
        ...
        if (!isValidEmail(newUser.email)) {
            cout << "Invalid email domain! Use @mail.jiit.ac.in\n";
            return false;
        }
        ...
        if (!isPasswordStrong(newUser.password)) {
            cout << "Password must be 8 characters or more, and must contain atleast one uppercase,\none lowercase, one number and one symbol.\n";
            return false;
        }
...
int main() {
    ...
    cout<<"Welcome to USER BLOG MANAGEMENT SYSTEM!"<<endl;
